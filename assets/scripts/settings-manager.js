...
class SettingsManager {
    ...
    shouldShowSettingFields(settingId) {
        ...
        // 规则2：已完成但未启用，字段也不显示
        const isEnabled = this.isSettingEnabled(settingId, state.config);
        if (!isEnabled) {
            console.log(`🔍 ${settingId} 已完成配置但未启用，不显示字段`);
            return false;
        }
        
        // 规则3：完成且启用之后，可以显示字段（具体哪些字段显示由字段值决定）
        console.log(`🔍 ${settingId} 已完成配置且已启用，可以显示字段`);
        return true;
    }
